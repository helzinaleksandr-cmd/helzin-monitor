...
def live_chart_section(coin, tf, market_type):
    ...
    if df is not None:
        ...
        m1.metric("Текущая цена", f"${price_now:,.2f}")
        m2.metric("Чистая прибыль", f"${total_pnl:,.2f}", delta=f"{total_pnl:,.2f}")
        m3.metric("Win Rate", f"{winrate:.1f}%")
        m4.metric("Открытые сделки", open_trades)
...
def render_equity_curve(period_label):
    st.subheader(f"📈 Кривая доходности ({period_label})")
    closed = [t for t in st.session_state.trades if "PROFIT" in t['Статус'] or "LOSS" in t['Статус']]
    if not closed:
...
if not st.session_state.logged_in:
    ...
    with st.form("login"):
        if st.form_submit_button("Войти как admin"): # Для теста упростил вход
    ...
    with st.sidebar:
        ...
        with st.form("trade_form", clear_on_submit=True):
            ...
            if st.form_submit_button("ОТКРЫТЬ", use_container_width=True):
                try:
                    e, a, s, tk = float(ent), float(amt), float(sl), float(tp)
                    st.session_state.trades.append({"id": datetime.now().timestamp(), "Время": datetime.now().strftime("%H:%M:%S"),
                        "Тип": side, "Монета": coin, "Кол-во": a, "Вход": e, "Stop": s, "Тейк": tk, 
    ...
    with t1:
        ...
        with c3:
            ...
            for i, f in enumerate(["5m", "15m", "1h", "4h", "1d"]):
                if bc[i].button(f, type="primary" if st.session_state.timeframe == f else "secondary", use_container_width=True):
